...
export default function Header() {
  ...
  // If menu is open, force header to be transparent/dark to match overlay
  const headerClass =
    ...
    <>
      <header className={headerClass}>
        <div className="flex justify-between items-center max-w-[1800px] mx-auto">
          ...
          <div
            onClick={() => router.push("/")}
            className="font-serif font-bold text-xl cursor-pointer relative z-50"
          >
            CURATED.
          </div>
            ...
            {user ? (
              <>
                ...
                <button
                  ...
                  className="font-sans text-xs tracking-widest hover:text-[#9F8155]"
                >
          ...
          >
            {mobileMenuOpen ? (
              <X size={24} weight="light" className="text-ink" /> // Close Icon
            ) : (
              <List size={24} weight="light" className="text-current" /> // Hamburger
            )}
          </button>
        </div>
      </header>
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.div
            ...
            className="fixed inset-0 bg-[#Fdfbf7] z-40 flex flex-col justify-center items-center gap-8 text-ink"
          >
            {/* Mobile Links */}
            <nav className="flex flex-col items-center gap-8">
              <button
                onClick={() => router.push("/")}
                className="font-serif text-4xl hover:text-[#9F8155] transition-colors"
              >
                Home
              </button>
              <button
            ...
            <div className="w-12 h-[1px] bg-gray-300 my-4"></div>

            {/* Mobile Auth */}
            {user ? (
              <button
                onClick={handleLogout}
                className="font-sans text-xs uppercase tracking-widest text-red-400"
              >
                Log Out
              </button>
            ) : (
              <button
                onClick={() => router.push("/login")}
                className="font-sans text-xs uppercase tracking-widest border border-ink px-8 py-3"
              >
                Login / Join
              </button>
            )}
